...
from slingshot.config import API_KEY, CONFIG, APIKey
...
# SLI-19250: Update with C1 Auth
class SlingshotAuth(httpx.Auth):
    ...
    def auth_flow(self, request: httpx.Request) -> Generator[httpx.Request, httpx.Response, None]:
        with self._sync_lock:
            ...
            if not self.cached_token.is_access_token_valid:
                raise TryAgain()
        request.headers["Auth"] = f"Bearer {self.cached_token.access_token}"
        yield request
...
# SLI-19250 replace with C1 Client
class SlingshotClient(RetryableHTTPClient):
    def __init__(
        self,
        api_url: str,
        api_key: APIKey,
        ...
        super().__init__(
            client=httpx.Client(
                ...
                auth=SlingshotAuth(api_url, api_key, access_token_cache_cls=access_token_cache_cls),
                timeout=60.0,
            )
        )
    ...
    def _send(self, request: httpx.Request) -> dict:
        ...
        return {"error": {"code": "Slingshot API Error", "message": "Transaction failure"}}


class AsyncSlingshotClient(RetryableHTTPClient):
    def __init__(
        self,
        api_url: str,
        api_key: APIKey,
...
_slingshot_client: Optional[SlingshotClient] = None


def get_default_client() -> SlingshotClient:
    global _slingshot_client
    if not _slingshot_client:
        _slingshot_client = SlingshotClient(
            CONFIG.api_url,
            API_KEY,
            access_token_cache_cls=get_access_token_cache_cache(),
        )
    return _slingshot_client


_async_slingshot_client: Optional[AsyncSlingshotClient] = None


def get_default_async_client() -> AsyncSlingshotClient:
    global _async_slingshot_client
    if not _async_slingshot_client:
        _async_slingshot_client = AsyncSlingshotClient(
            CONFIG.api_url,
            API_KEY,
            access_token_cache_cls=get_access_token_cache_cache(),
        )
    return _async_slingshot_client
