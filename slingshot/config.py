...
class APIKey(BaseSettings):
    id: str = Field(..., alias="api_key_id", env="SLINGSHOT_API_KEY_ID")
    secret: str = Field(..., alias="api_key_secret", env="SLINGSHOT_API_KEY_SECRET")
...
class Configuration(BaseSettings):
    # SLI-19250 Find new url for this
    api_url: str = Field("https://api.synccomputing.com", env="SLINGSHOT_API_URL")
...
class DatabricksConf(BaseSettings):
    ...
    aws_region_name: str = Field("us-east-1", env="DATABRICKS_AWS_REGION")
...
def set_api_key(api_key: APIKey):
    ...
    if _api_key is not None:
        raise RuntimeError(
            "Slingshot API key/secret has already been set and the library does not support resetting "
            "credentials"
        )
    _api_key = api_key
...
def _get_config_dir() -> Path:
    return Path("~/.slingshot").expanduser()
